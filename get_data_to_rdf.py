...
def get_data_to_rdf_object(input_path_data: str, output_path_data: str):
    ...
    with open(input_path_data) as f:
        ...
        for ind in df_objects.index.tolist():
            pos = len(lines) - lines.index("    // General axioms\n") + 4  # -36
            comment = f'    <!-- http://www.semanticweb.org/tinou/ontologies/2022/2/untitled-ontology-20#OBJ{ind + 1} -->\n'
            owl_first_line = f'    <owl:NamedIndividual rdf:about="http://www.semanticweb.org/tinou/ontologies/2022/2/untitled-ontology-20#OBJ{ind + 1}">\n'
            elt_rdf_type_nature = df_objects.loc[ind, "Nature d\'objets"]
            rdf_type_nature = f'        <rdf:type rdf:resource="http://www.semanticweb.org/tinou/ontologies/2022/2/untitled-ontology-20#{syntax_to_owl(elt_rdf_type_nature)}"/>\n'
            rdf_property = f'        <hasBeenFoundHere rdf:resource="http://www.semanticweb.org/tinou/ontologies/2022/2/untitled-ontology-20#{(unidecode.unidecode(str(df_objects.loc[ind, "Gare"]))).replace(" ", "_")}"/>\n'
            ...
            lines.insert(-pos, rdf_type_nature)
...
def get_data_to_rdf_train_station(input_path_data: str, output_path_data: str):
    ...
    with open(input_path_data) as f:
        ...
        for ind in df_place.index.tolist():
            pos = len(lines) - lines.index("    // General axioms\n") + 4  # -36
            comment = f'    <!-- http://www.semanticweb.org/tinou/ontologies/2022/2/untitled-ontology-20#{(unidecode.unidecode(df_place.loc[ind, "Intitulé gare"])).replace(" ", "_")} -->\n'
            owl_first_line = f'    <owl:NamedIndividual rdf:about="http://www.semanticweb.org/tinou/ontologies/2022/2/untitled-ontology-20#{(unidecode.unidecode(df_place.loc[ind, "Intitulé gare"])).replace(" ", "_")}">\n'
            rdf_type = f'        <rdf:type rdf:resource="http://www.semanticweb.org/tinou/ontologies/2022/2/untitled-ontology-20#Train_station"/>\n'
...
if __name__ == '__main__':
    file_rdf = 'data/test_context.owl'
